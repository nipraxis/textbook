...
variables defined in the [Global and local scope of Python
variables](https://matthew-brett.github.io/teaching/global_scope.html). In our
case the function `cost_function` can access variables `shifted_mid_vol1` and
`mid_vol0` that we defined in the top level (global) scope of our interactive
session:
...
matrices](https://en.wikipedia.org/wiki/Rotation_matrix)), so here is the 3D
optimization with the three translation parameters:
