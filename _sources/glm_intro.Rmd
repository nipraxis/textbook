...
a straight line of formula `0.9 * clammy + 10`.
...
where $\approx$ means *approximately equal to*, $b$ is the slope and $c$
is the intercept. For the guessed line above:

$y_i \approx 0.9 x_i + 10$
...
sy_y_val = Matrix(psychopathy)
Eq(sy_y, sy_y_val, evaluate=False)
```
...
Eq(sy_x, sy_x_val, evaluate=False)
```
...
sy_e_val = Matrix([sy_e_ind[i] for i in range(1, 13)])
