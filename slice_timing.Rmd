...
n_x = n_z_slices * 1.5
times = np.reshape(np.arange(n_z_slices), (2, -1)).T.ravel()
picture = np.repeat(times[:, None], n_x, axis=1)
cm = matplotlib.colors.LinearSegmentedColormap.from_list(
    'light_gray', [[0.4] * 3, [1] * 3])
plt.imshow(picture, cmap=cm, origin='lower')
plt.xticks([])
plt.yticks(np.arange(n_z_slices))
plt.xlabel('Bottom slice')
plt.ylabel('Position in space (0 = bottom)')
for i, time in zip(range(n_z_slices), times):
    plt.text(n_x / 2, i, str(time), va='center')
plt.title('''\
