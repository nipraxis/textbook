...
```{python}
fobj = open("important_notes.txt", "wt")
type(fobj)
```
...
```{python}
fobj = open("important_notes.txt", "rt")
print(fobj.readline())
print(fobj.readline())
```
...
Close a file when you have finished with it:
...
We may want to delete the file when we've finished with it.  There are utilities for things like that.  Here I'm using the `unlink` function from the `os` module (library) to delete the file:

```{python}
# Load the os module (library) so we can use it.
import os
```

```{python}
# Delete the file.
os.unlink('important_notes.txt')
```
