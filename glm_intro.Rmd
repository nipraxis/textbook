...
This page starts with the model for simple regression, that you have already see in the [regression page](on_regression.Rmd).
We then show how the simple regression gets expressed in a *design matrix*.
...
step is where multiple regression becomes the General Linear Model (GLM).
...
Image we have measured scores for a “psychopathy” personality trait in 12
...
You have already seen a preliminary guess we made at a straight line that links the `clammy` scores to the `psychopathy` scores.  Our guess was:
...
a straight line of formula `10 + 0.9 * clammy`.


```{python}
predicted_psychopathy = guessed_intercept + guessed_slope * clammy
predicted_psychopathy
```

Here are the data and our predictions from the guessed line.
...
Our next step is to write out this model more formally in mathematical
symbols.

$\newcommand{\yvec}{\vec{y}} \newcommand{\xvec}{\vec{x}} \newcommand{\evec}{\vec{\varepsilon}}$

To make this more general, let’s call our `psychopathy` data
$\yvec$ - a *vector* with 12 values, one for each student.   A *vector* is just a sequence of values.
...
student (= 0.389) and $x_i$ is the value for student $i$ where $i \in 1 ..
12$.
...
where $\approx$ means *approximately equal to*, $c$ is the intercept and $b$
is the slope. For the guessed line above:

$y_i \approx 10 + 0.9 x_i$

With the $\approx$, we are accepting that we will not succeed in explaining
...
Bear with with us for a little trick. If $\mathbf{o}$ is a vector of ones,
...
of ones ($\mathbf{o}$ above) and the second column is $\xvec$. Call $\bvec$
the column vector:
...
to the remaining error ($\yvec - \yhat$). If the design is orthogonal to the
errors, then $\Xmat^T \evec$ should be a vector of zeros.
