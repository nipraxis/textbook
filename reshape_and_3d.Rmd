...
```{python}
# rehaping to a three by three 2D array, notice that numpy fills the rows of the new array first, and then the columns
...
print(arr_2d.size)
print(arr_2d_3_by_2.size)

# same size?
arr_2d.size == arr_2d_3_by_2.size
```
...
```{python}
# reshape to an array with 3 rows, and whatever number of columns fits, given the size of the array
arr_1d.reshape((3, -1))
```

```{python}
# reshape to an array with 2 columns, and whatever number of rows fits, given the size of the array
arr_1d.reshape((-1, 2))
```
...
Here are the four planes of the array (slices over the third dimension) as NumPy shows them:
...
Again, compare the NumPy outputs to the image above, to makes sure you can visualize the array from both perspectives.
