...
In the case above, our second `import simplemod` triggers the shortcut, where
Python already has the module, so it returns the code from that module.  That
is the code that existed the first time we did `import simplemod`, so we get
the result of the long way round load of the original code.

Usually Python's mechanism is what we want, because we usually don't find
...
we are importing it again, the shortcut saves a lot of time.

For the rare case where we do edit the module while the Python process is
running, we can force the long way round, using a `reload`, like this:
